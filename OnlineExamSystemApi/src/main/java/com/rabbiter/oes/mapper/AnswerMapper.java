...
public interface AnswerMapper {
    @Select("select questionId, question, subject, score, section,level, \"选择题\" as type from multi_question where question like concat('%',#{question},'%') and subject like concat('%',#{subject},'%') and section like concat('%',#{section},'%')" +
            ...
            "union select questionId,question, subject, score, section,level, \"简答题\" as type from short_answer_question where question like  concat('%',#{question},'%') and subject like concat('%',#{subject},'%') and section like concat('%',#{section},'%')")
     ...
     * @return 简答题
     */
    @Select("select questionId, subject, question, answer, analysis, level, section from short_answer_question where questionId = #{questionId}")
    ShortAnswerQuestion findShortAnswerQuestionById(Long questionId);
}
